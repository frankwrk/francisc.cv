...
**Superpowers** — Methodology plugin that provides skills for brainstorming, TDD, verification-before-completion, code review, plan writing, and disciplined workflows. Recommended for this repo so agents follow structured processes (e.g. brainstorm before build, verify before claiming done).

**To add Superpowers:**

1. In Cursor, open **Settings** (e.g. Cmd+,) → **Features** (or **Tools & MCP**).
2. Open the **Plugins / Marketplace** panel and search for **Superpowers**.
3. Click **Install** (or **Enable** if it is already installed).
4. Optionally in **Rules**, set Superpowers skills to **Agent Decides** or **Always** as desired.

Plugins are installed at the user or project level from the Cursor Marketplace; there is no project file that installs them automatically.
