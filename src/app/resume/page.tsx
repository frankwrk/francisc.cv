...
function ResumeContent() {
  ...
  return (
    <article className="max-w-2xl space-y-10 pt-2 [font-family:var(--font-geist-sans)]">
      ...
      <section className="space-y-2">
        ...
        <p className="text-[15px] leading-7 text-[var(--scaffold-ruler)]">
          {resumeData.summary}
        </p>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-5">
          {resumeData.experience.map((entry) => {
            const isHighlighted = selectedRole === null || entry.roles.includes(selectedRole);
            return (
              <div
                key={entry.company}
                className={cn(
              ...
              >
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <h3 className="text-[15px] font-medium text-[var(--scaffold-toggle-text-active)]">
                    {entry.title}
                  </h3>
                  <span className="text-[11px] tracking-[0.06em] text-[var(--scaffold-ruler)] [font-family:var(--font-geist-pixel-square)]">
                    {entry.period}
                  </span>
                </div>
                <p className="mb-2 text-[13px] text-[var(--scaffold-ruler)]">
      ...
      <section className="space-y-3">
        ...
        <div className="border border-[var(--scaffold-line)]">
          {resumeData.skills.map((row, i) => (
            ...
            >
              <span className="font-medium text-[var(--scaffold-toggle-text-active)]">
                {row.area}
              </span>
              <span className="text-[var(--scaffold-ruler)]">{row.capabilities}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-2">
        ...
        <div className="border-l-2 border-[var(--scaffold-line)] pl-4">
          <p className="text-[15px] font-medium text-[var(--scaffold-toggle-text-active)]">
            {resumeData.education.degree}
          </p>
          <p className="text-[13px] text-[var(--scaffold-ruler)]">
            {resumeData.education.institution} · {resumeData.education.year}
          </p>
          <p className="mt-1 text-[13px] leading-relaxed text-[var(--scaffold-ruler)]">
      ...
      <div className="pb-4 pt-2">
        ...
        >
          <span className="text-sm font-medium tracking-[0.01em]">{downloadLabel}</span>
        </HoverBorderGradient>
      </div>
    </article>
  );
